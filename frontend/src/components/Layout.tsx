...
export function Layout() {
  ...
  const exitExam = () => {
    ...
    setGuard(null); setDestination(null); navigate(target);
  };
  return <StudyExitGuardContext.Provider value={{ setGuard, requestExit }}><div className={dark ? 'app dark' : 'app'}>
    ...
    : <div className="modal-backdrop"><section className="modal exam-exit-modal" role="dialog" aria-modal="true" aria-labelledby="exam-exit-title"><p className="eyebrow">모의고사 종료</p><h2 id="exam-exit-title">모의고사를 종료하시겠습니까?</h2><p>총 {guard.totalCount}문제 중 {guard.answeredCount}문제를 풀었습니다.</p><p className="unanswered-list"><b>미응답 문제</b><span>{guard.unansweredNumbers.length ? guard.unansweredNumbers.join(', ') : '없음'}</span></p><div className="exam-confirm-actions"><button className="button danger-button" onClick={exitExam}>시험 종료</button><button onClick={() => setDestination(null)}>취소</button></div></section></div>)}</StudyExitGuardContext.Provider>;
}
