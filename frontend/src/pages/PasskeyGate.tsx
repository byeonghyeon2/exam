...
export function PasskeyGate({ user }: { user: User }) {
  ...
  const registration = user.passkey_registration_required;
  ...
  return <main className="login-page"><section className="login-card passkey-card">
    <div className="login-brand"><span>CE</span><div><b>CertExam</b><small>시험 준비의 흐름</small></div></div>
    <div className="login-icon"><Fingerprint /></div>
    <p className="eyebrow">Device verification</p>
    <h1>{registration ? 'Passkey 등록' : 'Passkey 인증'}</h1>
    ...
    {passkey.isError && <p className="form-error" role="alert">{passkey.error.message}</p>}
